...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/VPSARG/VPS-ARG-2.0/master/blockBT.sh?token=AQD3IKK4HIGXE53TTYOM2LC7FY5UI"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} No se puede vincular a Github !" && exit 0
	wget https://raw.githubusercontent.com/VPSARG/VPS-ARG-2.0/master/blockBT.sh?token=AQD3IKK4HIGXE53TTYOM2LC7FY5UI -O /etc/ger-frm/blockBT.sh &> /dev/null
...
echo -e "  Panel de Firewall 🇦🇷 VPS-ARGENTO 🇦🇷${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}"
msg -bar2
