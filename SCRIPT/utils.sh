...
sysctl -p /etc/sysctl.conf > /dev/null 2>&1
msg -ama "$(fun_trans "TCP Detenido Con Exito")!"
} || msg -ama "$(fun_trans "Cancelado")!"
fi
}
SquidCACHE () {
...
echo -e " $(fun_trans "FECHA LOCAL APLICADA!")"
}
resetiptables () {
...
dpkg -l | grep -i ^rc
msg -ama "Eliminando Paquetes Obsoloteos"
...
echo -ne "\033[1;32m [4] > " && msg -azu "LIMPIEZA PAQUETES OBSOLETOS"
echo -ne "\033[1;32m [5] > " && msg -azu "$(fun_trans "RESETEO IPTABLES")"
