...
#Leer la informacion de configuracion
Get_IP(){
...
Modify_config_protocol_param(){
	...
	if [[ -z "${match_edit}" ]]; then
		...
		echo -e "${Info} Parametros de negociacion del usuario (numero de dispositivos limite) modificados correctamente ${Green_font_prefix}[Port: ${ssr_port}]${Font_color_suffix} (Nota: puede tomar aproximadamente 10 segundos aplicar la ultima configuracion)"
	fi
}
...
Download_SSR(){
	...
	# unzip "manyuser.zip"
	# [[ ! -e "/usr/local/shadowsocksr-manyuser/" ]] && echo -e "${Error} Fallo la descompresion del servidor ShadowsocksR !" && rm -rf manyuser.zip && exit 1
...
msg -bar
		echo -e "yes para cambiar a ${Green_font_prefix}Modo detallado (registro de conexion + registro de errores)${Font_color_suffix}？[y/N]"
msg -bar
